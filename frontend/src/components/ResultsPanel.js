...
function CompsTable({ comps }) {
  ...
  const sorted = [...comps].sort((a, b) => ((a[sort.key] ?? -Infinity) - (b[sort.key] ?? -Infinity)) * sort.dir);
  ...
  return (
    <div className="table-wrap">
      <table className="comps-table">
        <thead>
          <tr>
            <th>Ticker</th><th>Company</th><th>Sub-sector</th>
        ...
        <tbody>
          {sorted.map((c, i) => (
            <tr key={c.ticker} className={i === 0 ? 'top-row' : ''}>
      ...
      {rows.map(r => {
        ...
        return (
          <div key={r.key} className="mult-card">
            ...
            <div className="mult-median">{fmtX(m.median)} <span className="mult-sub">median</span></div>
            <div className="mult-range">{fmtX(m.p25)} – {fmtX(m.p75)} <span className="mult-sub">IQR</span></div>
...
function ValuationOutput({ implied, overall_range }) {
  ...
  if (!methods.length) return <p className="no-data">Add EBITDA and gross margin for more valuation methods.</p>;
      ...
      {overall_range && (
        <div className="implied-box">
          <div>
            ...
            <div className="implied-range">{fmtM(overall_range.low)} – {fmtM(overall_range.high)}</div>
          </div>
...
export default function ResultsPanel({ results, loading, error }) {
  ...
  if (error) return <div className="results-panel error-state"><div className="error-icon">⚠️</div><p>{error}</p></div>;
  ...
  return (
    <div className="results-panel">
      <div className="stats-row">
        ...
        {overall_range && <StatCard label="Implied EV range" value={`${fmtM(overall_range.low)} – ${fmtM(overall_range.high)}`} sub="25th–75th pctl" />}
      </div>
      <div className="tabs">
        {[['comps','Comparable Companies'],['multiples','Multiples Summary'],['valuation','Implied Valuation']].map(([k, label]) => (
          <button key={k} className={`tab ${tab === k ? 'active':''}`} onClick={() => setTab(k)}>{label}</button>
        ))}
      </div>
      <div className="tab-content">
        {tab === 'comps'     && <CompsTable comps={comps} />}
        {tab === 'multiples' && <div><p className="section-desc">Multiples across the {comps_count} closest public comparables.</p><MultiplesTable multiples={multiples} /></div>}
